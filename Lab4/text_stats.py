...
def get_most_frequent_words(f):
    ...
    word_counts = sorted(word_counts.items(), key=lambda x: x[1], reverse=True)[:5]
    ...
    for this_word, this_count in word_counts:
        ...
        following_this_word_counts = dict(Counter([word_list[ind] for ind in following_this_word_ind]))
        following_this_word_counts = sorted(following_this_word_counts.items(), key=lambda x: x[1], reverse=True)[:3]
...
def text_summary(f, file=None):
    ...
    print(f"Number of words: {n_words}\nNumber of unique words: {n_unique_words}", file=file)
